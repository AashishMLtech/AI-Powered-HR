...
class GeminiClient(BaseLLMClient):
    def __init__(self, api_key: str, fallback: BaseLLMClient | None = None):
        self.api_key = api_key
        self.fallback = fallback or MockLLMClient()

    async def rewrite_jd(self, raw_text: str, title: str) -> str:
        return await self.fallback.rewrite_jd(raw_text, title)

    async def social_caption(self, platform: str, job_title: str, jd: str) -> str:
        return await self.fallback.social_caption(platform, job_title, jd)

    async def score_cv(self, resume_text: str, job_title: str, jd: str) -> dict:
        return await self.fallback.score_cv(resume_text, job_title, jd)

    async def assess_github(self, github_summary: dict, job_title: str) -> dict:
        return await self.fallback.assess_github(github_summary, job_title)

    async def ai_resume_flag(self, resume_text: str) -> dict:
        return await self.fallback.ai_resume_flag(resume_text)
